...
- **CLI / configuration** — impersonate first: `POST https://api.seliseblocks.com/iam/v4/auth/impersonate` with `{ targeted_tenant_id, refresh_token, client_id }` (`client_id` is the fixed constant `57214b67-aa9c-4307-92ab-a25e35180fac`; the single-use `refresh_token` is consumed on success) → `PTOK`. Then `x-blocks-key: <PTENANT>` + `Authorization: Bearer <PTOK>` + `projectKey: <PTENANT>`. Each service call uses the tenant that owns that service (verified: data/IAM and localization can live in different project tenants).
