...
- **`PTENANT`** — the target project's tenant id (from `Project/Gets`, or user-provided) → **`projectKey`** in bodies (and equivalent query params). **Not** `x-blocks-key` on configuration calls.
...
```
…and puts `projectKey: <PTENANT>` in the body. **Strict rule:** for configuring data/IAM/storage/localization, `x-blocks-key` is always the root tenant (`ACCOUNT_TENANT`); `PTENANT` scopes the project via `projectKey` only. 401 / `session_expired` → expired `PTOK`; renew via `POST /iam/v4/auth-token` then re-impersonate (see get-into-project).

> ⚠️ **Do not swap the keys.** Configuration happens **after impersonation** — `PTOK` carries project scope, but the **`x-blocks-key` header stays `ACCOUNT_TENANT`**. Sending `PTENANT` as `x-blocks-key` on admin/config calls is wrong. `projectKey` in bodies is always `PTENANT` (same as `targeted_tenant_id`). Not impersonated yet? Run get-into-project first.
...
- **Never define system/default fields** — Blocks auto-adds `ItemId`, `CreatedDate`, `CreatedBy`, `LastUpdatedDate`, `LastUpdatedBy`, `Language`, `OrganizationId`, and `Tags` to every entity schema. Define only your own domain fields; adding these yourself conflicts with the platform-managed ones.
...
- **`x-blocks-key` = `ACCOUNT_TENANT` on configuration calls; `projectKey` = `PTENANT`.** Never send `PTENANT` as `x-blocks-key` when configuring schemas — the impersonated token scopes the project.
...
- **NEVER `POST /configurations` on an agent-created project.** If the project was created via `Project/Create` with the data-gateway resource (`"resources":[{"resourceId":"data","name":"data-gateway","link":""}]` — the agent/API path), the default Blocks MongoDB is auto-provisioned. `GET /configurations` returns a populated `data` object. Calling `POST /configurations` on a project that already has a configured data source **permanently poisons it** — the duplicate config cannot be deleted and blocks the default database. Only use `POST /configurations` for portal-created projects where `GET /configurations` returns `data: null`, or when explicitly attaching a custom external database. See [configure-schema.md](flows/configure-schema.md) Step 1 for the full decision table.
