...
- **`/activate` is separate from SSO login.** Already-activated users sign in via SSO anytime — no activate step. Only users **created or invited** through the Blocks portal or API (inactive until they set a password) need `/activate` first. Wire that page only if your app uses invite-and-activate — see [flows/configure-oidc.md](flows/configure-oidc.md) and **[blocks-iam-account](../blocks-iam-account/SKILL.md)**.
