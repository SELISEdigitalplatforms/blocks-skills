...
description: "Implement SELISE Blocks SSO / OIDC login in a frontend codebase — the authorization-code flow via `/iam/v4/idp/initiate` → redirect to `iam.seliseblocks.com` → `/iam/v4/idp/callback` (which sets the session cookie). It also covers refreshing the short-lived access token from the frontend via `POST /iam/v4/oidc/token` (`grant_type=refresh_token`). Use whenever the user wants to ADD login/sign-in to a Blocks app, wire up SSO/OIDC on the client, build a login button, handle the OIDC redirect/callback, or refresh/renew the access token / keep the session alive after SSO — 'add a login button', 'implement Blocks SSO in React', 'handle the OIDC callback', 'wire up sign-in with Blocks', 'refresh the access token from the refresh token', 'silently renew the Blocks session'. Do NOT implement username/password login directly — Blocks login goes through this hosted authorization-code flow. The project's OIDC provider must already be configured (blocks-iam-sso-oidc-configuration); this skill is the app-side wiring."
---
...
- **Refresh the access token via `POST /iam/v4/oidc/token`.** Access tokens are short-lived (~5 min); to renew without re-login, POST a **form-encoded** body (`grant_type=refresh_token`, `client_id`, `refresh_token`) with the mandatory `x-blocks-key` header and `credentials: "include"`. Blocks rotates both tokens and **sets the new `access_token`/`refresh_token` as cookies**. Use it as the 401-retry path (refresh, then retry once) and keep the newest refresh token. See [flows/login-flow.md](flows/login-flow.md) step 5.
...
- **Refresh** uses `POST /iam/v4/oidc/token` (form-encoded, `grant_type=refresh_token`) — see the key point above and [flows/login-flow.md](flows/login-flow.md) step 5. This is distinct from the direct-login refresh `POST /iam/v4/auth/refresh` used by username/password token flows (**blocks-iam-account** / **blocks-data-gateway-crud**); the hosted OIDC flow refreshes at `/oidc/token`.
