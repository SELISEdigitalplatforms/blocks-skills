...
**Optional — `/activate` (not part of login):** only for users **created or invited** via the Blocks portal or API who are still **inactive**. They activate once at `/activate` (`POST /auth/activate`), then use the SSO flow above like any other activated user. See **[blocks-iam-account](../blocks-iam-account/SKILL.md)**.
...
- **`/activate` is optional and separate from SSO login.** Already-activated users sign in via steps 1–4 with no activate step. Only inactive users invited via **[blocks-iam-users](../blocks-iam-users/SKILL.md)** (portal or API) need `/activate` once before their first login — see **[blocks-iam-account](../blocks-iam-account/SKILL.md)**.
