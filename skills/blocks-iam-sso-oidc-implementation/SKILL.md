...
- **Logout** for this hosted flow is the cookie/session call `POST /iam/v4/auth/Logout` (body `{}`, `credentials: "include"`) — it revokes and clears the session cookies this flow sets. See **[blocks-iam-account](../blocks-iam-account/SKILL.md)**. Token refresh mechanics are separate IAM session concerns.
