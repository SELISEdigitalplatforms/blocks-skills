...
**Signup and project creation are portal-only.** There is no API for either — do not try to script them. Hand off explicitly: tell the user exactly what to do in the browser (see [flows/first-time-setup.md](flows/first-time-setup.md)), ask them to tell you when they're done, then **verify by re-running preflight** rather than taking "done" on faith.
...
1. **Which project + environment** (offer the preflight list).
...
- **An empty `Project/Gets` is not an error** — it means "no project yet"; route to project creation, don't retry.
