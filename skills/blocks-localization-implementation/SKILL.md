...
- **Don't "fix" these to the `…Cloud…` swagger names.** The current localization swagger lists only `Language/GetCloudstLanguages`, `Module/GetCloudsModules`, and `Key/GetCloudUilmFile` — but `/Language/Gets`, `/Module/Gets`, and `/Key/GetUilmFile` are still served and are the **correct** frontend endpoints (verified live 2026-07-13: they return 200 with the **public project key**, whereas the `…Cloud…` variants **401 with the project key** — they're admin/impersonation-scoped). Swagger omits the runtime read routes here just as it omits the data gateway; keep using the `/Gets` / `GetUilmFile` names.
